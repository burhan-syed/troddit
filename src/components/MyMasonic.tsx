  ...
  return (
    <div>
      <InfiniteScroll
        ...
        hasMore={feed.hasNextPage === true ? true : false}
        loader={<>{feed.hasNextPage && feed.isFetching && context?.infiniteLoading && (
      ...
      >
        ...
        >
          {items.map((post, i) => (
            <div key={post?.data?.name ?? i}>
              {PostCard({ data: post, index: i })}
            </div>
          ))}
        </Masonry>
      </InfiniteScroll>
